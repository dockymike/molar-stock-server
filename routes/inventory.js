...
router.get('/', verifyToken, async (req, res) => {
  ...
  try {
    let baseQuery = `
      ...
      FROM inventory inv
      LEFT JOIN inventory_categories c ON inv.category_id = c.id
...
router.post('/add', verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log('📥 Incoming inventory add payload:', req.body)
    ...
    for (const item of supplies) {
      ...
      if (item.isNew) {
        ...
        if (invRows.length > 0) {
          ...
          const insert = await client.query(
            'INSERT INTO inventory (name, barcode, user_id, created_at, updated_at) VALUES ($1, $2, $3, NOW(), NOW()) RETURNING id',
            [name, barcode || null, userId]
          )
      ...
      console.log(`✅ Added ${item.quantity} of inventory ID ${inventoryId} to location ID ${targetLocationId}`)
    }
...
router.put('/:id', verifyToken, async (req, res) => {
  ...
  try {
    console.log(`📦 Updating inventory ID ${id} with payload:`, req.body)
    await client.query('BEGIN')
...
router.post('/consume', verifyToken, async (req, res) => {
  ...
  try {
    ...
    console.log('📥 Incoming inventory consume payload:', req.body)
    ...
    for (const item of supplies) {
      ...
      console.log(`✅ Consumed ${quantityToConsume} from inventory ID ${inventoryId} at location ${location}`)
    }
